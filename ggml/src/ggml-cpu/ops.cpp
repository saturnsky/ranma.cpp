...
// ggml_compute_forward_dsv4_hc_coef

static void ggml_compute_forward_dsv4_hc_coef_f32(
        const ggml_compute_params * params,
        ggml_tensor * dst) {
    const ggml_tensor * mixes = dst->src[0];
    const ggml_tensor * scale = dst->src[1];
    ...
    for (int64_t it = it0; it < it1; ++it) {
        ...
        for (int64_t ih = 0; ih < hc; ++ih) {
            const float xv = *(const float *) ((const char *) mixes->data + ih*nbm0 + it*nbm1);
            const float bv = *(const float *) ((const char *) base->data  + ih*nbb0);
            const float v = xv * scale_pre + bv;
            *(float *) ((char *) dst->data + ih*nbd0 + it*nbd1) = 1.0f/(1.0f + expf(-v)) + eps;
        }

        // post: 2*sigmoid(mixes*scale[1] + base)
        for (int64_t ih = 0; ih < hc; ++ih) {
            const int64_t idx = hc + ih;
            const float xv = *(const float *) ((const char *) mixes->data + idx*nbm0 + it*nbm1);
            const float bv = *(const float *) ((const char *) base->data  + idx*nbb0);
            const float v = xv * scale_post + bv;
            *(float *) ((char *) dst->data + idx*nbd0 + it*nbd1) = 2.0f*(1.0f/(1.0f + expf(-v)));
        }
...
void ggml_compute_forward_dsv4_hc_coef(
        const ggml_compute_params * params,
        ggml_tensor * dst) {
    const ggml_tensor * src0 = dst->src[0];
    ...
    switch (src0->type) {
        case GGML_TYPE_F32:
            {
                ggml_compute_forward_dsv4_hc_coef_f32(params, dst);
            } break;
        default:
            {
                GGML_ABORT("fatal error");
