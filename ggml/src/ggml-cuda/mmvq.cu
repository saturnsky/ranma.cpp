...
bool ggml_cuda_should_use_mmvq(enum ggml_type type, int cc, int64_t ne11) {
    ...
    if (GGML_CUDA_CC_IS_RDNA4(cc)) {
        switch (type) { // tuned on Radeon AI PRO R9700 (gfx1201) with rows_per_block 4 for 3..8 columns
            case GGML_TYPE_Q4_K:
            case GGML_TYPE_Q5_K:
            case GGML_TYPE_Q6_K:
                // MMQ cost is flat from 3 rows and beats the multi-row MMVQ from 5 rows, MMVQ wins at 1..4 rows.
                return ne11 <= 4;
            default:
                return ne11 <= MMVQ_MAX_BATCH_SIZE;
        }
...
static constexpr __host__ __device__ int calc_rows_per_block(int ncols_dst, int table_id, bool small_k = false, int nwarps = 1) {
    ...
    if (table_id == MMVQ_PARAMETERS_RDNA4) {
        ...
        // Two columns are faster with one row per block. Tuned on gfx1201, gfx1200 not measured.
        switch (ncols_dst) {
            case 3:
            case 4:
            case 5:
