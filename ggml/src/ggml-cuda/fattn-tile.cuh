...
static __global__ void flash_attn_tile(
    ...
    // With a gather the same pointer carries the per-query index lists instead of a per-tile KV bound.
    ...
    static_assert(!use_sparse || ncols1 == 1, "sparse gather needs one query row per tile");
...
#pragma unroll
    ...
    const int32_t * GGML_CUDA_RESTRICT indices = use_sparse ?
        sparse_indices + (int64_t(sequence % ne33)*ne31 + col_Q_0)*ne11 : nullptr;
    if constexpr (use_sparse) {
        // All ncols2 columns belong to the same query token, so they share one index list.
        // ne11 is the index budget here; slots past the end of the list are -1 and drop out.
...
static bool ggml_cuda_fattn_tile_shall_use_sparse(const int device, const ggml_tensor * dst) {
    ...
    if (K->ne[1] < std::max<int64_t>(min_kv, 2LL*n_kv_max)) {
        return false;
    }
    ...
    if (!logged && getenv("GGML_CUDA_FATTN_SPARSE_LOG")) {
        logged = true;
        GGML_LOG_INFO("%s: gathering %d of %lld cells (D %lld, %lld query rows, GQA %lld)\n", __func__,
                n_kv_max, (long long) K->ne[1], (long long) Q->ne[0], (long long) Q->ne[1],
...
// Launch the gathering kernel variant for one query row. Returns false if the shape does not qualify,
// in which case the caller keeps its dense dispatch.
...
static bool launch_fattn_tile_sparse(ggml_backend_cuda_context & ctx, ggml_tensor * dst,
    ...
    if (!ggml_cuda_fattn_tile_shall_use_sparse(id, dst)) {
        return false;
    }
...
static void launch_fattn_tile_switch_ncols2(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    if constexpr (DKQ == 256 && DV == 256) {
        if (hip_fattn_gqa12 && use_gqa_opt && Q->ne[1] == 1 && gqa_ratio % 12 == 0) {
            ...
            const int nwarps    = ggml_cuda_fattn_tile_get_nthreads (DKQ, DV, ncols1*ncols2, cc) / warp_size;
            const int nbatch_fa = ggml_cuda_fattn_tile_get_nbatch_fa(DKQ, DV, ncols1*ncols2, cc);
            if (launch_fattn_tile_sparse<DKQ, DV, ncols2, use_logit_softcap>(ctx, dst, id, cc, warp_size)) {
                return;
            }
