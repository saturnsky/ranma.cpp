...
public:
    ...
    // Makes `selected` resident. Synchronous on the copy stream; the caller has drained the compute
    // streams, so nothing reads the arena or the tables while this runs.
    bool install(const std::vector<std::vector<int32_t>> & selected, l1_install_stats & stats);
