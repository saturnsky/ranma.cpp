...
struct test_dsv4_hc_coef : public test_dsv4_hc {
    const int64_t n_tokens;
    const int32_t n_iter;
    const float eps;
    ...
    test_dsv4_hc_coef(int64_t n_tokens = 17, int32_t n_iter = 4, float eps = 1e-6f)
        : n_tokens(n_tokens), n_iter(n_iter), eps(eps) {}
    ...
    ggml_tensor * build_graph(ggml_context * ctx) override {
        ...
        out = ggml_dsv4_hc_coef(ctx, mixes, scale, base, eps, n_iter);
        ggml_set_name(out, "out");
        return out;
    }
};
...
struct test_top_k_block : public test_case {
    ...
    // live cells; -1 leaves a few cells short of n_kv, varying per stream
    const int64_t n_live;
    // cells in position order, as a text cache fills them, instead of shuffled
    const bool    ordered;
    ...
    std::string vars() override {
        std::string res = VARS_TO_STR8(n_kv, ratio, n_tps, n_stream, k, described, mask_type, stable_ties);
        if (n_live >= 0) {
            res += "," + VAR_TO_STR(n_live);
        }
        if (ordered) {
            res += "," + VAR_TO_STR(ordered);
        }
        return res;
    }
    ...
    test_top_k_block(int64_t n_kv = 8192, int64_t ratio = 4, int64_t n_tps = 1, int64_t n_stream = 1,
            ...
            int64_t n_live = -1, bool ordered = false)
        : n_kv(n_kv), ratio(ratio), n_tps(n_tps), n_stream(n_stream), k(k), described(described),
          mask_type(mask_type), stable_ties(stable_ties), n_live(n_live), ordered(ordered) {}
    ...
    void initialize_tensors(ggml_context * ctx) override {
        ...
        for (int64_t s = 0; s < n_stream; ++s) {
            ...
            const int64_t n_live = this->n_live >= 0 ? std::min(this->n_live, n_kv) :
                std::max<int64_t>(1, n_kv - 3 - (s*37)%11);
...
static std::vector<std::unique_ptr<test_case>> make_test_cases_eval() {
    ...
    // hc_coef: the same three sources as hc_comb, but the result also carries the
    // pre/post coefficients. Production shapes: hc = 4, n_iter = 20, decode (1) and a 512 batch.
    test_cases.emplace_back(new test_dsv4_hc_coef(1, 20));
    test_cases.emplace_back(new test_dsv4_hc_coef(512, 20));
...
// Conv_2D test cases
 ...
 // A deep layer of a ConvNet, several images in the batch
    ...
    // the layout a text cache has: cells in position order, the padded tail of the cell window
    // empty, the newest token the query. Every remainder of the live count modulo the ratio,
    // no spare block at all (every cell in a full block), and prompt rows that carry the meta
    for (int64_t n_kv : {8192, 65536}) {
        for (int64_t tail : {0, 1, 2, 3}) {
            test_cases.emplace_back(new test_top_k_block(n_kv, 4, 1, 1, 2051, true,  GGML_TYPE_F16, true, n_kv - 200 + tail, true));
        }
        test_cases.emplace_back(new test_top_k_block(n_kv, 4, 1, 1, 2051, false, GGML_TYPE_F16, true, n_kv - 199, true));
        test_cases.emplace_back(new test_top_k_block(n_kv, 4, 1, 1, 2051, true,  GGML_TYPE_F16, true, n_kv, true));
        test_cases.emplace_back(new test_top_k_block(n_kv, 4, 1, 2, 2051, true,  GGML_TYPE_F16, true, n_kv - 37, true));
        test_cases.emplace_back(new test_top_k_block(n_kv, 4, 8, 1, 2051, true,  GGML_TYPE_F16, true, n_kv - 101, true));
    }
    // just above the budget: the spare block's listed cells decide the threshold
    test_cases.emplace_back(new test_top_k_block(2304, 4, 1, 1, 2051, true, GGML_TYPE_F16, true, 2054, true));
    test_cases.emplace_back(new test_top_k_block(2304, 4, 1, 1, 2051, true, GGML_TYPE_F16, true, 2051, true));
    ...
    // one query row over a GQA-12 head group with F16 K/V (256-wide heads): the HIP tile kernel has a 12-column path for it
    test_cases.emplace_back(new test_flash_attn_ext(256, 256, 2, {12, 1},  512, 1, true, false, 0, 0, GGML_PREC_F32, GGML_TYPE_F16, GGML_TYPE_F16));
    test_cases.emplace_back(new test_flash_attn_ext(256, 256, 2, {12, 1}, 8192, 1, true, false, 0, 0, GGML_PREC_F32, GGML_TYPE_F16, GGML_TYPE_F16));
    ...
    // HIP tile gather (qwen4 shape - gqa 12): the batch size picks the tile width, so it decides how many
    // query rows share one index list. 1 row per tile is the GQA-12 decode block, 2 and 4 rows come out of
    // the generic column switch. Every width is covered with one and, where the tile is widest, with two
    // sequences, at a cell count where the gate max(4096, 2*rows*budget) is open, and once with a budget
    // far below the cell count so that the lists are short.
    test_cases.emplace_back(new test_flash_attn_ext(256, 256, 2, {12, 2}, 8192,  1, true, false, 0, 0, GGML_PREC_F32, GGML_TYPE_F16, GGML_TYPE_F16, {0, 1, 2, 3}, true, false,  512));
    test_cases.emplace_back(new test_flash_attn_ext(256, 256, 2, {12, 1}, 8192,  2, true, false, 0, 0, GGML_PREC_F32, GGML_TYPE_F16, GGML_TYPE_F16, {0, 1, 2, 3}, true, false,  512));
    test_cases.emplace_back(new test_flash_attn_ext(256, 256, 2, {12, 1}, 8192,  4, true, false, 0, 0, GGML_PREC_F32, GGML_TYPE_F16, GGML_TYPE_F16, {0, 1, 2, 3}, true, false,  512));
    test_cases.emplace_back(new test_flash_attn_ext(256, 256, 2, {12, 2}, 4096,  4, true, false, 0, 0, GGML_PREC_F32, GGML_TYPE_F16, GGML_TYPE_F16, {0, 1, 2, 3}, true, false,  512));
    test_cases.emplace_back(new test_flash_attn_ext(256, 256, 2, {12, 2}, 8192,  4, true, false, 0, 0, GGML_PREC_F32, GGML_TYPE_F16, GGML_TYPE_F16, {0, 1, 2, 3}, true, false,  256));
    ...
    // Qwen QSA: 256/256, gqa 12, budget 2048, one query row per tile. 4096 cells is exactly where the gate
    // opens for that budget, and the two-sequence case is the one the model runs with several slots.
...
static std::vector<std::unique_ptr<test_case>> make_test_cases_perf() {
    ...
    // qwen4exp QSA block selection of one decode token, with the block meta (single-row
    // selection) and without it (the general walk over every cell)
    for (int64_t n_kv : {8192, 32768, 65536}) {
        for (bool described : {false, true}) {
            test_cases.emplace_back(new test_top_k_block(n_kv, 4, 1, 1, 2051, described, GGML_TYPE_F16, false, n_kv - 199, true));
        }
    }
