...
- **The fused `MUL_MAT_ID` path** (models with a quantized expert down bias or scale) reads the
  arenas too. It notifies the host tier after the fused kernel has finished reading the down
  weights; the controller checks kind 2, so gate and up fusions do not release the layer's slots.
